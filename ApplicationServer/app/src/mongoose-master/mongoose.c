...
static int capture_len(struct frozen *f, int token_index, const char *ptr) {
	...
	EXPECT(token_index >= 0 && token_index < f->max_tokens,
	       JSON_STRING_INVALID);
...
static int parse_value(struct frozen *f) {
	...
	switch (ch) {
		...
		case '9':
			...
			break;
		default:
			return ch == END_OF_STRING ? JSON_STRING_INCOMPLETE
			                           : JSON_STRING_INVALID;
	}
       ...
       */
			...
			if (ch == 's') {
				const char *s = va_arg(ap,
				                       const char *); /* Always fetch parameter */
				int j;
				int pad = field_width -
				...
				if (s != NULL) {
					...
					for (j = 0; (precision <= 0 || j < precision) &&
					            s[j] != '\0'; j++) {
...
MG_INTERNAL struct mg_connection *mg_create_connection(
     ...
     */
		...
		if (!mg_if_create_conn(conn)) {
			MG_FREE(conn);
			conn = NULL;
			MG_SET_PTRPTR(opts.error_string, "failed start connection");
		}
	} else {
...
void mg_if_recv_udp_cb(struct mg_connection *nc, void *buf, int len,
     ...
     */
		for (nc = mg_next(lc->mgr, NULL);
		     nc != NULL; nc = mg_next(lc->mgr, nc)) {
...
void mg_broadcast(struct mg_mgr *mgr, mg_event_handler_t cb, void *data,
	...
	/*
   * AppServer manager has a socketpair, `struct mg_mgr::ctl`,
	...
	if (nc->flags & MG_F_CONNECTING) {
		if (fd_flags != 0) {
			...
			if (!(nc->flags & MG_F_UDP)) {
				...
				int ret =
						getsockopt(nc->sock, SOL_SOCKET, SO_ERROR,
						           (char *) &err, &len);
				if (ret != 0) err = 1;
			}
#else
...
static void mg_mgr_handle_ctl_sock(struct mg_mgr *mgr) {
	struct ctl_msg ctl_msg;
	int len =
			(int) MG_RECV_FUNC(mgr->ctl[1], (char *) &ctl_msg, sizeof(ctl_msg),
			                   0);
...
time_t mg_mgr_poll(struct mg_mgr *mgr, int timeout_ms) {
   ...
   */
	...
	for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
		...
		if (nc->sock != INVALID_SOCKET) {
			if (num_ev > 0) {
				fd_flags = (FD_ISSET(nc->sock, &read_set) ? _MG_F_FD_CAN_READ
				                                          : 0) |
				           (FD_ISSET(nc->sock, &write_set) ? _MG_F_FD_CAN_WRITE
				                                           : 0) |
...
static void mg_http_transfer_file_data(struct mg_connection *nc) {
	...
	if (pd->file.type == DATA_FILE) {
		...
		size_t to_write =
				left <= 0 ? 0 : left < (int64_t) io->len ? (size_t) left
				                                         : io->len;
...
static size_t mg_http_parse_chunk(char *buf, size_t len, char **chunk_data,
	...
	while (i < len && isxdigit(s[i])) {
		n *= 16;
		n += (s[i] >= '0' && s[i] <= '9') ? s[i] - '0' : tolower(s[i]) - 'a' +
		                                                 10;
		i++;
	}
      ...
      return;
		...
		if ((req_len < 0 ||
			...
			mg_websocket_handler(nc, MG_EV_RECV, ev_data);
		           ...
		           (vec = mg_get_http_header(hm, "Sec-WebSocket-Key")) !=
		           NULL) {
			...
			int trigger_ev = nc->listener ? MG_EV_HTTP_REQUEST
			                              : MG_EV_HTTP_REPLY;
	...
	while ((ch = fgetc(fp)) != EOF) {
		if (in_ssi_tag && ch == '>' && buf[len - 1] == '-' &&
			...
			if (memcmp(p, d_include.p, d_include.len) == 0) {
				mg_do_ssi_include(nc, path, p + d_include.len + 1,
				                  include_level, opts);
...
static void mg_handle_move(struct mg_connection *c,
	...
	if (dest == NULL) {
		...
		if (p != NULL && p[1] == '/' &&
		    (p = (char *) memchr(p + 2, '/', dest->p + dest->len - p)) !=
		    NULL) {
...
static void mg_handle_put(struct mg_connection *nc, const char *path,
	...
	if ((rc = mg_create_itermediate_directories(path)) == 0) {
		...
		const struct mg_str *range_hdr = mg_get_http_header(hm,
		                                                    "Content-Range");
		int64_t r1 = 0, r2 = 0;
...
MG_INTERNAL int mg_uri_to_local_path(struct http_message *hm,
	...
	*local_path = NULL;
	...
	{ /* 1. Determine which root to use. */
		...
		while ((rewrites = mg_next_comma_list_entry(rewrites, &a, &b)) !=
		       NULL) {
			if (a.len > 1 && a.p[0] == '@') {
				/* Host rewrite. */
...
static void mg_prepare_cgi_environment(struct mg_connection *nc,
	...
	mg_addenv(blk, "SERVER_SOFTWARE=%s/%s", "AppServer", MG_VERSION);
  ...
  addenv(blk, "REMOTE_PORT=%d", ri->remote_port);
	...
	if ((h = mg_get_http_header((struct http_message *) hm,
	                            "Content-Length")) !=
	    NULL) {
       ...
       */
			if (nc->flags & MG_F_USER_1) {
				...
				if (len < 0 || io->len > MG_MAX_HTTP_REQUEST_SIZE) {
					...
					} else if ((h = mg_get_http_header(&hm, "Status")) !=
					           NULL) {
...
MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path,
	...
	is_cgi =
			(mg_match_prefix(opts->cgi_file_pattern,
...
static int mg_http_common_url_parse(const char *url, const char *schema,
	...
	while (*url != '\0') {
		*addr = (char *) MG_REALLOC(*addr,
		                            addr_len + 5 /* space for port too. */);
		if (*addr == NULL) {
			DBG(("OOM"));
			return -1;
		}
...
struct mg_connection *mg_connect_ws(struct mg_mgr *mgr,
	...
	if (mg_http_common_url_parse(url, "ws://", "wss://", &use_ssl, &addr,
	                             &port_i,
	                             &path) < 0) {
...
void mg_hexdump_connection(struct mg_connection *nc, const char *path,
	...
	fprintf(
			fp, "%lu %p %s %s %s %d\n", (unsigned long) time(NULL), (void *) nc,
			src,
...
int mg_match_prefix_n(const char *pattern, int pattern_len, const char *str,
	...
	for (; i < pattern_len; i++, j++) {
		if (pattern[i] == '?' && j != str_len) {
			...
			do {
				res = mg_match_prefix_n(pattern + i, pattern_len - i,
				                        str + j + len,
				                        str_len - j - len);
			} while (res == -1 && len-- > 0);
...
int mg_parse_dns(const char *buf, int len, struct mg_dns_message *msg) {
	...
	for (i = 0;
	     i < msg->num_answers && i < (int) ARRAY_SIZE(msg->answers); i++) {
...
static void dns_handler(struct mg_connection *nc, int ev, void *ev_data) {
	...
	switch (ev) {
		case MG_EV_RECV:
			...
			if (mg_parse_dns(nc->recv_mbuf.buf, nc->recv_mbuf.len, &msg) ==
			    -1) {
